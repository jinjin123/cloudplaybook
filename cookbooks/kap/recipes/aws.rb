...
if (not (defined?(kylin)).nil?) && (not "#{kylin}" == "")
  ...
  if awsaction.include?("create")
    ...
    execute "checkvpcforgateway" do
      command "subnetid=$(aws emr describe-cluster --cluster-id #{emrid} --query 'Cluster.Ec2InstanceAttributes.Ec2SubnetId'| cut -d '\"' -f2);echo \"Subnetid = \"$subnetid >> #{basedir}aws/#{identifier}/deploy.log;vpccheckcommand=$(aws ec2 describe-subnets --query \'Subnets[? SubnetId==\`$subnetid\` ].VpcId\' --output text);vpcid=eval $vpccheckcommand;echo \"Vpcid = \"$vpcid >> #{basedir}aws/#{identifier}/deploy.log;checkgatewayattachcommand=$(aws ec2 describe-internet-gateways --query \'InternetGateways[*].Attachments[? VpcId == \`$vpcid\`].VpcId\' --output text);gatewayresult=eval $checkgatewayattachcommand;echo \"GatewayResult = \"$gatewayresult >> #{basedir}aws/#{identifier}/deploy.log;if [ -z \"$gatewayresult\" ];then exit 1;fi"
    end
  end
end
