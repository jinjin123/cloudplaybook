...
if (not (defined?(kylin)).nil?) && (not "#{kylin}" == "")
  ...
  if awsaction.include?("create")
    ...
    execute "run_install" do
      command "echo \"Running accountregion = #{accountregion}\">> #{basedir}aws/#{identifier}/deploy.log;ssh -t -i #{basedir}aws/#{identifier}/credentials/kylin.pem -o StrictHostKeyChecking=no ec2-user@`aws cloudformation describe-stacks --stack-name #{identifier}-chefserver --query 'Stacks[*].Outputs[*]' --output text | grep ServerPublicIp| awk {'print $NF'}` \"sudo /root/create_client.sh #{identifier} #{instancetype} #{accountregion}\" >> #{basedir}aws/#{identifier}/deploy.log"
    end
