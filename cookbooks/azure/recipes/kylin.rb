...
execute "createimageifnotexist_removecontainerifexist" do
    execute "if [ `docker images|grep \'^#{image_name}$\'|wc -l` -nq \'1\' ];then docker commit #{container_name} #{image_name};fi;if [ `docker ps -a|grep \'^#{container_name}$\'|wc -l` -eq \'1\' ];then docker stop #{container_name}||true;docker rm #{container_name}|fi"
end
